...
r_protein_from_AGB_kg_ha <-
  (r_agb_hydeareas_kg_km2/100) / r_fcr # div by 10o to convert per km2 to per ha

names(r_protein_from_AGB_kg_ha) <- c("dairy", "beef", "sheep_and_goats") #, "average"

plot(r_protein_from_AGB_kg_ha) # sheep and goats missing from this
```
...
f_energy_from_protein_MMkcal_ha <- function(animalcategory_fao, animalcategory_FCR) {
  ...
  r_protein_production_animals <-
    subset(r_protein_from_AGB_kg_ha, animalcategory_FCR) 
...
r_protein_from_AGB_kg_perpix <-
  r_protein_from_AGB_kg_ha * cellSize(r_protein_from_AGB_kg_ha, unit = "ha") * 
  r_fraction_gl
global(r_protein_from_AGB_kg_perpix, fun = "sum", na.rm = T)/1e9 # dairy 49 mmt, beef and sheep 11 mmt

  ## outliers removed
f_global_sum_without_outliers(r_protein_from_AGB_kg_perpix$dairy) /1e9 # 41 dairy
f_global_sum_without_outliers(r_protein_from_AGB_kg_perpix$beef) /1e9 # 9 beef
f_global_sum_without_outliers(r_protein_from_AGB_kg_perpix$sheep_and_goats) /1e9 # 9 sheep_and_goats
...
r_kcal_animals_MM_perpix <- 
  r_kcal_animals_MM_ha * cellSize(r_kcal_animals_MM_ha, unit ="ha") *
  r_fraction_gl
global(r_kcal_animals_MM_perpix*1e6, fun = "sum", na.rm = T)/1e12 # dairy 505 trillion kcal

  ## outliers removed
f_global_sum_without_outliers(r_kcal_animals_MM_perpix$dairy_MMkcal_ha) /1e6 # 440 dairy
f_global_sum_without_outliers(r_kcal_animals_MM_perpix$beef_MMkcal_ha) /1e6 # 100 beef
f_global_sum_without_outliers(r_kcal_animals_MM_perpix$sheep_and_goats_MMkcal_ha) /1e6 # 151 sheep_and_goats





# livestock protein masked to cropland areas 
