...
(plt_fcrs <- 
  ...
  # add FCR of current herd structures
  # remember to discuss in the text that this current herd structure fcr was not used when calculating protein yields -- it just illustrates how it will look like
# (plt_fcrs_also_current_str <- 
#     create_index_map(r_index =  c(r_fcr, r_fcr_current_herd_str),
#                      tocrs = "ESRI:54030",
#                      index_main_title = "Feed conversion ratios (FCR) by animal species",
#                      index_label = "[kg AGB / kg protein]",
#                      colorpal = pal_fcr,
...
(plt_animalprotein_hydeareas <- 
    create_index_map(r_index = r_protein_from_065xAGB_kg_ha, 
                     tocrs = "ESRI:54030",
...
- LUR
Those maps could also be masked to areas where there is cropland and in croplands with SI < 10


!! should also total protein be presented? It is interesting that production from marginal lands is just 4.3 / 11.1 = 39%

! lisää vähintäänkin appendixiin proteiinikartta niiden alueiden ulkopuolella, jotka ovat croplandia
Ehkä myös kartat
...
names(r_combined_protein_plots) <- c("prot_kgha", "prot_kgha_clmask", "prot_kgha_SI<10clmasl")
...
names(r_shares_plot) <- c(
  "liv_shre_tot_prot", "liv_shre_tot_prot_clmask", "liv_shre_tot_prot_SI10clmasl")
# ,
...
# LUR
# combine LUR rasters
r_LUR_current_combined <- 
  c(LUR_i_current,
    LUR_i_current_croplandmask,
#    LUR_i_current_outside_croplands, # this is not needed
    LUR_i_current_SIbelow10mask_croplandmask)
names(r_LUR_current_combined) <-   c("LUR", "LUR_cl_mask", "LUR_SIbelow10clmask")
...
(plt_LUR_current <- # works when LUR layers have different names
  create_index_map(r_index = r_LUR_current_combined, 
                   tocrs = "ESRI:54030",
                   index_main_title = "",
                   index_label = "[LUR]",
                   colorpal = pal_lur,
...
tmap_save(plts_combined_prot_and_shares_and_LUR, filename = here("Figures", "plts_combined_prot_and_shares_and_LUR.pdf"))
```
