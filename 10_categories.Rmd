...
prot_yield_change_bot <- -0.25
prot_yield_change_top <-  0.25
...
# tutkitaan muutosta current to AlltoCL 
  # jos kasvaa > +25% niin nykyisen gl tulisi muuttua croplandiksi 1111 = grazing to cropland (Major)
  # jos muutos + - 25% ei muutosta -7777 = cropland remains cropland (No change)
  # jos muutos > - 25% niin ei kannata muuttaa grazing landia croplandiksi, vaan CltoGl 10000 = cropland to grazing (Major) <-- eipäs vaan harmful conversation
...
rcl_prot_changeing_90overCL <- 
  c(-Inf, prot_yield_change_bot, 10000, # jos proteiinisato pienenee 25% tai enemmän kun 90% cl alueet muutetaan kokonaan cl niin cl:ksi muuttaminen haitallista (älä lisää cl) --> mistä tiedetään pitääkö muuttaa GL vai pitääkö vain pitää nykyisellään? Olisiko otsikko 'ALLtoCL change harmful' paras
    prot_yield_change_bot, prot_yield_change_top, -7777, # Jos muutos vähemmän kuin +-25% niin pidä croplandina
    prot_yield_change_top, Inf, 1111) %>%  # Jos muutos  +25% tai enemmän niin kannattaa muuttaa gl to cl 1111 = grazing to cropland (Major)
...
# tutkitaan muutosta current to AlltoCL where >90% GL
  # jos muutos > - 25% niin ei kannata muuttaa grazing landia croplandiksi alueilla 90% gl, vaan CltoGl (10000 = cropland to grazing (Major)) TAI PIDÄ ENNALLAAN as GL --> paras kirjoita harmful conversation if AlltoCL in GL90% areas
  # jos kasvaa > +25% niin nykyisen gl tulisi muuttua croplandiksi 1111 = grazing to cropland (Major)
  # jos muutos + - 25% ei muutosta -10000 = grazing remains grazing (No change)
...
rcl_prot_changeing_90overGL <- 
  c(-Inf, prot_yield_change_bot, 10000, # jos prot sato pienenee 25% tai enemmän niin gl-cl muuttaminen haitallista (älä lisää cl ) TAI PIDÄ ENNALLAAN? --> ehkä paras kirjoittaa converting to ALLtoCL is harmful in areas where 90% gl
    prot_yield_change_bot, prot_yield_change_top, -10000,  # Jos muutos vähemmän kuin +-25% niin pidä grazing landina
    prot_yield_change_top, Inf, 1111) %>%  # Jos muutos  +25% tai enemmän niin kannattaa muuttaa gl to cl = T)
  matrix(., ncol=3, byrow = T)
...
# tutkitaan muutosta current to AlltoCL where mixed
  # jos muutos > - 25% niin ei kannata muuttaa mixed landia croplandiksi --> pidetään mixed ennallaan tai mix-gl (mutta ei tiedetä tätä) --> voisi kirjoittaa AlltoCL harmful in mixed areas
  # jos muutos + - 25% ei muutosta -555 = mixed remains mixed (No change)
  # jos kasvaa > +25% niin nykyiset mixedit muutetaan croplandiksi
...
rcl_prot_changeing_mixed <- 
  c(-Inf, prot_yield_change_bot, 10000, # jos prot sato pienenee 25% tai enemmän niin cl muuttaminen haitallista (pidä mix tai tee jotain muuta) --> or write conversion AlltoCL harmful in mixed areas eli 10000
    prot_yield_change_bot, prot_yield_change_top, -555,  # Jos muutos vähemmän kuin +-25% niin pidä mixed mixedinä -555 = mixed remains mixed (No change)
    prot_yield_change_top, Inf, 1) %>%  # Jos muutos +25% tai enemmän niin kannattaa muuttaa 1 = mixed to cropland (Moderate)
  matrix(., ncol=3, byrow = T)
...
(plt_r_prot_recoded_changes_combined_ALLtoCL <- 
  create_index_map(r_index = r_prot_recoded_changes_combined_ALLtoCL,
                 ...
                 index_label = "Direction of change. If protein production changes < ±25%, land use remains same)",
...
rcl_prot_changeing_harmful <- 
  c(-Inf, prot_yield_change_bot, 0, # jos prot sato pienenee 25% tai enemmän niin gl-gl muuttaminen haitallista -- pidä gl as gl
    prot_yield_change_bot, prot_yield_change_top, 0,  # Jos muutos vähemmän kuin +-25% niin pidä grazing landina mutta tässä matriisissa sellaiset saavat arvon = 0 koska yhdistämme myöhemmin
    prot_yield_change_top, Inf, 10000) %>%  # Jos muutos  +25% tai enemmän niin kannattaa muuttaa gl to gl tai mix to gl)
  matrix(., ncol=3, byrow = T)
...
#                      index_label = "Direction of change. If changes in protein production are less than  ±25%, land use remains the same",
#                      colorpal = pal_categories,
#                      # Päivitä raja-arvot ja luokkanimet vastaamaan todellisia arvoja
#                      breakvals = c(-Inf, -9000, -7000, -500, 10, 2000, Inf), # poistettu luokka 5000 luokan 10 jälkeen
#                      breaknames = breaknames_updated))
...
tmap_save(plt_r_prot_recoded_changes_combined_ALLtoCL,
          filename = here("Figures", "Figure 6 Categories - where land usage should change or not.pdf"))
...
writeRaster(r_prot_recoded_changes_combined_ALLtoCL,
            filename = here("Data", "Intermediate_input",
                            "r_prot_recoded_changes_combined_ALLtoCL.tif"))
```
...
tmap_save(plt_r_prot_recoded_changes_combined_ALLtoCL_whereExpansion_ge75,
          filename = here("Figures", "Figure 6 Categories - masked where cl expansion ge 75.pdf"))
