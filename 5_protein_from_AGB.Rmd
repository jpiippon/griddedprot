...
# Total protein produce
  # with current: 10.6 mmt
global(r_protein_from_AGB_kg_perpix, fun = "sum", na.rm = T)/1e9 # 10.6427
...
(1000 * global(r_protein_from_AGB_kg_perpix, fun = "sum", na.rm = T)) / # gram
  ...
  365  # days in a year
# 4.2 g/capita/d with current herd structures

    # compare this to daily requirements (http://dx.doi.org/10.1016/S0140-6736(18)31788-4)
    # average person weight = 70kg and protein need = 0.8 per kg
((1000 * global(r_protein_from_AGB_kg_perpix, fun = "sum", na.rm = T)) / # gram
  (1000* foodBalanceSheets_semiraw %>% filter(Area == "World", Item == "Population") %>% pull(Y2010)) / # persons
  365 ) /# days in a year
  (0.8*70) # 8%
    ...
    # in 2010 people would need 56g * 365d = 20440 g. Our esimate would be enough for 
(1000 * global(r_protein_from_AGB_kg_perpix, fun = "sum", na.rm = T)) /
    (0.8*70*365) / # 20440
  1e6 
...
# Rasterize protein fraction - not needed!
