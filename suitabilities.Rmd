...
# ----------------------- chunk 3 laskettu uusilla datoilla
...
# 
#  ---------------- some more rasters
  # can plot chunk 5 after these three rasters
...
r_cropland_expansion_percent_outside_croplands <-
  ...
  # can plot chunk 6 after these
# 
# 
# r_grazingland_reduction_percent <-
#   here("Data", "Intermediate_input","r_grazingland_reduction_percent.tif") %>%
#   rast()
# 
...
#   ("Data", "Intermediate_input","r_grazingland_reduction_percent_outside_croplands.tif") %>%
#   rast()
# 
# 
# 
...
SI_30arcsec <- 
  here("Data", "Input", "Zabel22_SI", 
       "overall_suitability_subset_1to17_hist1980_2009_current_irr_areas_applied.bil") %>% # changed!!
...
# optimised dairy protein production = original protein raster - raster showin how much dairy prot should decrease (increses if there are negative values in r_protein_decrease_basedOnSI_dairy_kg_perpix)
...
# 
writeRaster(r_optimised_protein_yield_whendairy_combined_with_crops_kg_perpix,
            filename = here("Data", "Intermediate_input",
                            "r_optimised_protein_yield_whendairy_combined_with_crops_kg_perpix.tif"),
            overwrite= T)

writeRaster(r_optimized_protein_yield_larger_percent,
            filename = here("Data", "Intermediate_input",
                            "r_optimized_protein_yield_larger_percent.tif"),
            overwrite= T)
...
# global sums --- these are the same as above
# df_SI <- df_SI %>% 
#   mutate(potential_grazingland_reduction_Mha = 
#            pull(global(r_potential_grazingland_reduction_ha_perpix, 
#                        fun = "sum", na.rm = T)/1e6),
#          potential_grazingland_reduction_cl_mask_Mha = 
#            pull(global(r_potential_grazingland_reduction_croplandmask_ha_perpix,
#                        fun = "sum", na.rm = T)/1e6),
#          potential_grazingland_reduction_outside_cl_Mha = 
#            pull(global(r_potential_grazingland_reduction_outside_croplands_ha_perpix,
#                        fun = "sum", na.rm = T)/1e6))

  # dairy protein
df_SI <- df_SI %>% 
  mutate(Original_dairy_protein_production_mmt = 
           pull(global(r_protein_from_065xAGB_kg_perpix$dairy, "sum", na.rm=T)/1e9),
         Protein_decrease_basedOnSI_dairy_mmt =
...
# vanha selitys alla -- ei välttis validi
# minne häviää noin 50% proteiinista?
# silloinkin jos vain SI maa-alueet, joiden SI > 95 kelpuutetaan uusiksi viljapelloiksi, maa-ala kasvaa "vain" 50mha
# onko mahdollista, että jos gl käytöstä poistuu 50Mha, proteiini tippuu 27mmt --> 13 mmt? Ei varmaan
# onko joku outo SI maski taustalla?
