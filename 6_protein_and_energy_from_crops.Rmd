...
global(r_physical_areas_crops_sum_ha_perpix, fun = "sum", na.rm = T)/1e6 # 9.8 milj km2 total --> higher than Total global cropland area estimate of
...
r_fraction_cl[r_fraction_cl >1] <- 1 # cannot be over 1
...
r_prot_allcrops_sum_mt_perpix <- 
  terra::resample(r_prot_allcrops_sum_mt_perpix, template_rast_5arcmin) 
...
r_kcal_allcrops_sum_MM_ha <- 
  ...
  (cellSize(r_kcal_allcrops_sum_MM_perpix, unit = "ha") * r_fraction_cl_0toNA)
summary(r_kcal_allcrops_sum_MM_ha)  # med 7 mean 8
...
global(r_prot_allcrops_sum_mt_perpix, fun = "sum", na.rm = T)/1e6 # 274 mmt -- incl outliers
...
# see our world in data   https://ourworldindata.org/food-supply
  ...
  ##  outliers removed 
# f_global_sum_without_outliers(r_prot_allcrops_sum_mt_perpix)/1e6 # 240 mmt
# f_global_sum_without_outliers(r_kcal_allcrops_sum_MM_perpix)/1e6 # 8359 trillion kcal
...
names(r_kcal_allcrops_sum_MM_perpix) <- "crop_kcal_MM_perpix"
